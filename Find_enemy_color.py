...
def find_Enemy_Color():#색상 찾기 코드 정리본
    # 영상 불러오기
    try:
        print('영상을 불러옵니다.')
        name = e.filename()
    ...
    #    ret, screen = cap.read()
    #   lower, upper =  find_Colors(screen)
    # 다시 0초부터 video 재생
...
def find_Colors(screen):
    # 색상추출
    rgb = check_Colors(screen)
...
def show_frame(frame, lower, upper):
    # mask와 나머지 설정
